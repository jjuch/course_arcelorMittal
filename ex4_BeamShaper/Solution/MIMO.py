...
def MIMO_decoupler(model, tf_x, tf_y, plot=False):
    ...
    '''
    # DC gains of the the model with inputs u_x, u_y and outputs f_x, f_y
    G_full = control.dcgain(model)
    G = np.array([[G_full[1, 0], G_full[1, 3]], [G_full[3, 0], G_full[3, 3]]])
    G_w = np.diag(np.diag(G)) # Only diagonal elements should be kept to avoid cross-coupling

    # Obtain the decoupling matrix
    Gd = np.linalg.inv(G) @ G_w

    # Create state space model of the process with decoupler (no closed-loop needed)
    B_new = np.block([[tf_x.B, np.zeros_like(tf_x.B)],
                    [np.zeros_like(tf_y.B), tf_y.B]]) @ Gd
    D_new = np.zeros((4, 2))
